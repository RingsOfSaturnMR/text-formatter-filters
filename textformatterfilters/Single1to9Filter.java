 ...
 * @author mattroberts
 */
public class Single1to9Filter implements UnaryOperator<TextFormatter.Change> {

    @Override
    public TextFormatter.Change apply(TextFormatter.Change textFormatter) {

        if (textFormatter.isReplaced()) {
            if (Character.toString(textFormatter.getControlNewText().charAt(0)).matches("[1-9]")) {
            ...
            if (textFormatter.getControlText().length() > 0) {
                textFormatter.setText("");
            } else if (Character.toString(textFormatter.getControlNewText().charAt(0)).matches("[1-9]")) {
                textFormatter.setText(Character.toString(textFormatter.getControlNewText().charAt(0)));
            } else {
                textFormatter.setText("");
            }
